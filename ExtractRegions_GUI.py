...
class DrawRegions(QGraphicsPixmapItem):
	...
	def saveRegion(self):
		fname=QFileDialog.getSaveFileName(None,"Save Region",str(os.getcwd()))
		f=open(fname,'w')
		f.write("%d %d\n" %(graphX,graphY))
	...
	def loadRegion(self):
		fname=QFileDialog.getOpenFileName(None,"Load Region",str(os.getcwd()))
		f=open(str(fname))
	...
	def clearRegion(self):
		self.finalArray, self.spaceArray=[],[]
		self.minX, self.maxX=graphX,0
		self.minY, self.maxY=graphY,0
		self.update()
...
class MainWindow(QMainWindow):
	def __init__(self):
		...
		clearRegionAction=self.toolBar.addAction("Clear Region")
		self.toolBar.addSeparator()
		...
		clearRegionAction.triggered.connect(self.clearRegion)

	def clearRegion(self):
		self.imagePanel.clearRegion()		
	def showMsg01(self):
		self.mode=1
		self.statusBar.showMessage("Select Mode")
	def showMsg02(self):
	...
	def exportAsFits(self):
		if self.mode != 0:
			...
			out_fits=QFileDialog.getSaveFileName(None,"Save FITS file",str(os.getcwd()))
					...
					if self.mode==1:
						...
						if not TFtable[j,i]:
							if len(input_data[0,0])==3:
								...
								output_data[j,i]=[0,0,0,255]
			if len(input_data[0,0])==3:
				imOutput=Image.fromarray(output_data,mode='RGB')
			elif len(input_data[0,0])==4:
				imOutput=Image.fromarray(output_data,mode='RGBA')

			out_image=QFileDialog.getSaveFileName(None,"Save PNG Image",str(os.getcwd()),
	...
	def openImage(self):
		fname=QFileDialog.getOpenFileName(self,"Open image",
			str(os.getcwd()),"Image Files (*.bmp *.jpg *.png *.xpm)")
	...
	def exportWSLAP(self):
		...
		if self.mode!=0:
			...
			if self.mode==1:
				...
				original_data=copy.copy(input_data)		
			...
			targetX, targetY= 1400,1400

			# layer=1
			file_mode=1
			layer=self.editInput()
			print 'layer=',layer
			output_data=rebin(original_data,(targetX,targetY))

			wslapName=QFileDialog.getSaveFileName(self,"Save mass layer file",str(os.getcwd()))
			if file_mode==1:
				f=open(wslapName,'w')
			elif file_mode==2:
				f=open(wslapName,'a')
			for i in range(targetX):
				for j in range(targetY):
					f.write("\t\t%d\t\t%d\t%0.8f\t\t\t%d\n" %(i+1,j+1,output_data[j,i],layer))
			f.close()
			print 'done'
		else:
			self.warningMsg("Please choose select/mask mode")
	...
	def editInput(self):
		...
		num,ok=inputDialog.getInt(self,"choose layer number","",1,1,2147,1)
		if ok:
			return num
